...
class CSVW:
    def __init__(self, url=None, path=None, metadata_url=None, metadata_path=None, date_parsing=False):
        if url:
            ...
            self.handle = StringIO(response.read())
        elif path:
        ...
        if metadata_url:
            ...
            self.metadata_handle = StringIO(response.read())
        elif metadata_path:
            self.metadata_handle = open(metadata_path, 'rb')
        ...
        self.table_set = _build_tables(self.handle, url, date_parsing)
        self.metadata = meta.metadata_extraction(url, self.metadata_handle)
